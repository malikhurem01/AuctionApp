...
public class AuthenticationController {
    ...
    public ResponseEntity<?> getUserWithToken(final HttpServletRequest request) throws UsernameNotFoundException {
        ...
        User user = null;
        try {
            user = userServiceImplementation.loadUserByUsername(username);
    ...
    public ResponseEntity<?> refreshToken(final HttpServletRequest request,
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            try {
                ...
                final String username = decodedJWT.getSubject();
                final User user = userServiceImplementation.loadUserByUsername(username);
                new_access_token = userServiceImplementation.generateToken(user.getUsername(),
                        jwtConfig.getAccessTokenExpireTime(), request.getRequestURL().toString());
            } catch (Exception exc) {
                logger.error(exc.getMessage());
